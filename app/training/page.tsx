...
async function getTrainingCategories() {
  const query = `*[_type == "trainingCategory"] | order(order asc) {
    title,
    icon,
    image,
    description,
...
export default async function TrainingPage() {
  const sanityCategories = await getTrainingCategories();
  
  const categories = sanityCategories.length > 0 
    ? sanityCategories.map((cat: any) => ({
        ...cat,
        image: cat.image ? urlFor(cat.image).url() : cat.image
      }))
    : fallbackCategories;
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="py-16 px-8 md:px-12 lg:px-20 bg-gradient-to-b from-gray-50 to-white">
        <div className="max-w-7xl mx-auto">
          <TrainingGrid categories={categories} />
        </div>
      </section>
    </div>
  );
